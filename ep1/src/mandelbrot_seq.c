...
int main(int argc, char *argv[]){
    struct time_track input_time, computing_time, output_time;
    struct time_track alloc_time, dealloc_time, full_time;

    start_time_tracking(&full_time);

    start_time_tracking(&input_time);
    init(argc, argv);
    end_time_tracking(&input_time);
    
    start_time_tracking(&alloc_time);
    allocate_image_buffer();
    end_time_tracking(&alloc_time);

    start_time_tracking(&computing_time);
    compute_mandelbrot();
    end_time_tracking(&computing_time);

    start_time_tracking(&output_time);
    write_to_file();
    end_time_tracking(&output_time);

    start_time_tracking(&dealloc_time);
    free_image_buffer();
    end_time_tracking(&dealloc_time);

    end_time_tracking(&full_time);

    printf("%d,", image_size);
    printf("%3.4lf,", get_wall_time(&input_time) + get_wall_time(&output_time));
    printf("%3.4lf,", get_wall_time(&alloc_time) + get_wall_time(&dealloc_time));
