...
void init(int argc, char *argv[]){
    if (argc < 2) {
        printf("usage: ./mandelbrot_ompi_pth n_threads\n");
        exit(0);
    } 
...
void * mandelbrot_task(void *arg){
    ...
    int start = ((struct args *) arg)->start;
    int tasksize = ((struct args *) arg)->tasksize;
...
void compute_mandelbrot(int start, int blocksize) {
    int tasksize = blocksize / n_threads;

    for (int i = 0; i < n_threads; ++i) {
        struct args *Args = (struct args *) malloc(sizeof(struct args));
        Args->start = start + i * tasksize;
        Args->tasksize = tasksize;

        if (Args->start > image_buffer_size) {
            Args->tasksize = image_buffer_size - Args->start + 1;
        }
...
int main(int argc, char *argv[]){
    ...
    if (taskid == 0) {
        ...
        for (int i = 1; i < numtasks; ++i) {
            ...
            if (start + tasksize > image_buffer_size) {
                tasksize = image_buffer_size - start + 1;
            }
        ...
        compute_mandelbrot(start, tasksize);
    ...
    if (taskid == 0) {
        write_to_file();
        printf("%d,%d,%d,", image_size, numtasks, n_threads);
